...
### 2.18 환자 명단 '퇴원' 토글 바닥 고정 `@Coder-UI` — ✅ **완료 (2026-09-25)**

> **증상 (사용자)**: 좌측 환자 명단이 길면 맨 아래 '퇴원' 그룹이 화면 밖으로 밀려 누르기 힘들다.

- [x] 퇴원 머리줄을 스크롤 영역 바닥에 `sticky bottom-0`으로 고정 — 명단 길이와 상관없이 항상 보인다
  - `section`을 `contents`로 둬 sticky 기준이 스크롤 영역이 되게 함 (section 안에 두면 붙지 않는다)
  - 누르는 영역을 줄 전체·높이 40px(데스크톱 36px)로 키움
- [x] 바닥에서 눌러 펼치면 목록이 화면 밖에 열리므로, 머리줄 위치로 스크롤해 보여준다
- [x] 테스트 2건 — `PatientRail.test.tsx`
...
| 2026-09-25 | **퇴원 토글 바닥 고정 (2.18)**: 환자 명단이 길면 퇴원 그룹이 화면 밖으로 밀려 누르기 힘들었다. 퇴원 머리줄을 스크롤 영역 바닥에 sticky로 붙이고, 펼치면 그 위치로 스크롤한다. | ✅ 완료 | @Coder-UI |
