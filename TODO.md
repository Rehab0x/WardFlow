...
### 🔴 사용자 액션 필요
0. **마이그레이션 적용** — 둘 다 추가 전용이라 순서 상관없이 붙여넣고 실행하면 된다
   - `supabase/migrations/202609210001_patient_standing_orders.sql` (지시오더 칸)
   - `supabase/migrations/202609210002_patient_nutrition.sql` (키·체중·필요열량 설정)
   - `supabase/migrations/202609210003_patient_important_notes.sql` (환자별 중요사항)
   적용 전에는 해당 값 저장만 실패하고 나머지 기능은 정상 동작한다
1. ~~Supabase 마이그레이션 적용~~ — **2026-09-20 적용 완료** (알림 기능 동작 중)
2. **설정 > AI 설정** — 텍스트 LLM API 키 (+ 음성 기능을 쓰려면 Whisper 키)
3. **Lab 초기화** — `supabase/maintenance/reset-labs.sql`을 SQL 에디터에서 실행 (STEP 1 확인 → STEP 2)
   재입력 전에 Lab 수신함 "처리 기록 지우기"도 눌러야 같은 파일이 다시 처리된다
4. ~~이원의료재단 XLS 샘플~~ — **받아서 PT·VRE 두 건 수정 완료 (2026-09-20)**. 재입력하며 남은 오류 확인
...
### 🟢 다음 세션에 할 일
0. **Lab 파싱 정확도(2.9)** — XLS 샘플을 받는 즉시 착수. 한글 검사명 매핑 + Culture 오분류
