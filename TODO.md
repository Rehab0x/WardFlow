...
**마이그레이션**: 2026-09-21 기준 **전부 적용 완료** (대기 중인 것 없음)
...
### 🔴 사용자 액션 필요
- ~~마이그레이션 적용~~ — **2026-09-21 전부 적용 완료**
  (알림 · 지시오더 · 필요열량 · 중요사항)
- ~~Lab 초기화~~ · ~~이원 XLS 샘플~~ — 완료
- 남은 것 없음. 다음 세션은 **실사용에서 나온 것부터** 처리하면 된다
...
| 음성 질의 | 마이크 권한(특히 iOS Safari), 한국어 환자명 인식 정확도 | 3.3.1 |
| 환자명 발음 매칭 | 2026-09-20 추가된 자모 매칭이 실제 오인식을 얼마나 잡는지. 엉뚱한 환자로 잇지는 않는지 (임계값 `koreanName.ts`) | 3.3.1 · 3.3.2 |
| 간호사 대화 정리 | 여러 환자 섞인 대화가 제대로 나뉘는지 | 3.3.2 |
...
### 🟢 다음 세션에 할 일
0. **Lab 파싱 — 남은 오류 수집(2.9)**. PT·VRE는 고쳤다. 다음은 실제로 이상하게 들어온 행을 받아서:
   다른 Culture 조합(MRSA·객담·상처 등), 긴 한글 검사명이 그대로 남는 항목
1. 위 검증 결과 반영 (임계값 조정, 프롬프트 튜닝, 회진 뱃지 길이·글자 크기 등)
...
### 세션 기록 (2026-09-21) — 실사용하며 다듬기
LLM 모델 목록 갱신(은퇴 모델 정리) → SOAP A) Problem List 형식 → 지시오더 박스 →
1일 필요열량 계산기(+환자별 저장) → 중요사항 박스 → 환자 목록 기준일 →
**메모 탭이 지난 메모를 못 보던 버그 수정** → 메모 접기·날짜 지정 → 로고 홈 이동.
상세는 2.9~2.16 및 이슈 테이블 참고. **지표**: 테스트 392 → **427건**, 커밋 12개, 전부 배포 완료.

### 세션 기록 (2026-09-20 오후) — 실사용 피드백 반영
환자명 발음 매칭(오인식 보정) → 회진 모드 신설 → 뱃지 숫자·임계값 지속 표시 →
환자 목록 메모 필터 → 앱 아이콘 재디자인 → Lab 초기화 경로.
상세는 2.6~2.9 및 이슈 테이블 참고. **지표**: 테스트 277 → **339건**, 커밋 5개, 전부 배포 완료.

### 이전 세션(2026-09-19 ~ 20)에 한 일
...
### 2.6 회진 모드 (회진용 환자 명단) `@Coder-UI` `@Coder-Logic` — ✅ **구현 완료 (2026-09-20)**

> **배경 (사용자 요청)**: 회진 돌 때 좌측 환자 레일은 글씨가 작아 눈에 안 들어온다.
> 회진 중에는 "이 병동, 이 방, 이 환자, 오늘 뭐 볼 것" 네 가지만 크게 보이면 된다.

- [x] 병동 탭 — 병실번호 앞자리로 1병동(100번대) / 2병동 / 3병동 … 나눠 눌러서 전환 → `features/app/roundingList.ts`
  - [x] 병동 목록은 **실제 환자 병실에서 파생** (하드코딩 없음). 병실번호 없는 환자는 `기타` 탭에 모으고 항상 마지막
  - [x] 탭에 병동별 인원 + 플래그 있는 환자 수 표시
- [x] 101호 카드 안에 **환자 이름 + 플래그만 크게** → `components/rounding/RoundingRoomCard.tsx`
  - 이름 20px semibold, 행 높이 56px(터치 타겟 확보), 병상번호·성별/나이는 작게 보조
  - 진단명·주호소는 일부러 싣지 않는다 (워크스페이스에서 본다)
  - [x] 플래그는 기존 사이드바 인디케이터 재사용 — 주의 / 알림 / 일정 / 항생제 / Lab. **아이콘 대신 색+글자 뱃지**(회진 중에는 글자가 빠르다)
  - [x] 퇴원 환자 제외, 컨설트 환자는 포함(같이 돈다) + `컨설트` 뱃지
- [x] 환자 누르면 기존 워크스페이스로 진입, 뒤로 가면 **보던 병동 탭 그대로** (`mainView` + `roundingWard`를 AppPage가 보관)
- [x] 진입 경로 — 상단바 회진 버튼(데스크톱) + 모바일 하단 네비 4칸으로 확장
- [x] 테스트 35건 — `roundingList.test.ts`(11), `RoundingBoard.test.tsx`(8), `roundingBadges.test.ts`(10), `alertEngine.test.ts` 위반 판정(6)
- [x] **뱃지에 숫자 표시** (2026-09-20 사용자 요청) → `features/app/roundingBadges.ts`
  - [x] `Na 118↓` · `Lab 3` · `타조페란주 D+16` · `알림 2` · `일정 09:00` — 브리핑 집계를 그대로 조합, **추가 쿼리 없음**
  - [x] 길게 누르면/마우스 올리면 전체 설명 (규칙명, 검사일, 연속 횟수, 비정상 항목 목록)
- [x] **임계값 위반은 해결될 때까지 유지** (2026-09-20 사용자 요청) → `alertEngine.findOpenLabBreaches()`
  - 알림 이벤트(`alert_events`)는 "언제 터졌나"의 기록이라 시간이 지나도 남는다. 회진 뱃지는 **"지금도 문제인가"**여야 하므로 별도 계산한다
  - [x] 항목별 **가장 최근 검사**만 판정 — 아직 걸려 있으면 계속 뜨고, 다음 검사에서 값이 돌아오면 사라진다
  - [x] 그 항목을 재지 않은 검사는 건너뛴다 (오늘 Na를 안 냈으면 마지막 Na 값이 기준)
  - [x] 연속으로 걸린 시작일·횟수를 함께 계산해 설명에 표시
  - [x] Lab을 새로 넣으면 뱃지도 따라 바뀌도록 브리핑 갱신 시각을 평가 트리거에 연결 (`useAlertEvaluation.refreshKey`)
  - ⚠️ **판정 범위는 최근 14일**(`LAB_LOOKBACK_DAYS`). 그 전 검사가 마지막이면 뱃지가 뜨지 않는다
- [ ] **실사용 확인 필요** — 실제 병실 번호 체계(예: `101-1` / `1012` / 병동명 표기)와 글자 크기가 회진 중에 실제로 읽히는지
- [ ] **실사용 확인 필요** — 뱃지가 한 줄에 몇 개까지 감당되는지 (항생제 여러 개 + Lab + 알림이면 줄이 길어진다)

### 2.7 환자 목록 '메모' 필터 `@Coder-UI` — ✅ **구현 완료 (2026-09-20)**

> **배경 (사용자 요청)**: 대화 정리 SOAP이 메모(`type: 'progress'`)로 저장되므로,
> **금일 메모가 있는 환자**를 따로 볼 수 있으면 회진 중 "어디까지 적었나"를 짚을 수 있다.

- [x] 좌측 환자 목록 필터에 `메모` 추가 — 전체 / 주의 / 할 일 / **메모** (4칸)
- [x] 판정 기준: **오늘 작성된 경과기록**(`briefing.progressNotes`) — 대화 정리 SOAP도 같은 경로로 저장된다
- [x] `메모`는 **할 일 카운트에 섞지 않는다** — 이미 한 일이므로
- [x] 환자 행에도 표시 (`NotebookPen`, 에메랄드) — 전체 목록에서도 누가 적혔는지 보인다
- [x] 테스트 11건 — `PatientRail.test.tsx`(5), `patientIndexes.test.ts`(4), `roundingBadges.test.ts` 메모 뱃지(2)
- [x] 회진 모드 뱃지에도 `메모` 추가 (2026-09-20 사용자 요청) — 에메랄드 `done` 톤, **뱃지 줄 맨 뒤**
  - 임상 신호(주의·임계값·Lab·알림·일정·항생제)가 먼저, "이미 적었다"는 표시는 뒤. **뱃지가 없는 것이 곧 "아직 안 적음"**
  - 설명에는 SOAP 첫 줄만 (본문이 여러 줄이라)

### 2.8 앱 아이콘 재디자인 `@Coder-UI` — ✅ **완료 (2026-09-20)**

> **배경 (사용자 요청)**: 기존 아이콘(티얼 그라데이션 + W + 심전도선 + "FLOW" 글자)이 마음에 안 든다.
> 요소가 셋이라 서로 뭉개지고, 작은 크기에서 글자가 읽히지 않았다.

- [x] 시안 3종 제시 → **사용자 선택: 다크 + 티얼 W**
- [x] 마크는 **맥박이 그리는 W** 하나로 통일 — 글자를 빼서 16px에서도 형태가 남는다
- [x] `scripts/generate-icons.mjs` — 벡터 정의 하나에서 전 사이즈 생성 (`npm run icons`)
  - [x] `rounded` 브라우저·PWA / `square` iOS(자체 마스크가 있어 모서리를 둥글리면 두 번 잘림) / `maskable` 안드로이드(바깥 20% 잘림 대비 마크 72%)
  - [x] 기존에는 512 하나를 any·maskable에 같이 쓰고 있어 적응형 아이콘에서 모서리가 잘렸다 → 전용 파일 분리
- [x] 상단바 로고도 같은 마크로 교체 (`components/brand/WardFlowMark.tsx`, 기존 청진기 아이콘 대체)
- [ ] (선택) `index.html`의 `theme-color`(#0d9488)와 manifest `theme_color`(#18181b)가 서로 다르다 — 아이콘과 무관하게 예전부터 불일치

### 2.9 Lab 파싱 정확도 개선 (이원의료재단 XLS) `@Coder-Logic` — 🔨 진행 중 (2026-09-20)

> **배경 (사용자)**: 외부 에이전트(그록봇/헤르메스 등)로 이원의료재단 XLS를 받아 자동 입력할 계획.
> 파싱 오류를 고칠 일이 반복될 것이므로, **깨끗한 상태에서 다시 시작**하고 수정 경로를 정리한다.

- [x] **Lab 전체 초기화 경로** → `supabase/maintenance/reset-labs.sql`
  - 삭제 전 건수 확인(STEP 1) → 전체/사용자별 삭제 → 검증 → Lab 유래 알림 기록 정리
  - `lab_items`는 `lab_result_id` cascade로 함께 삭제됨. 환자·메모·투약·차팅은 그대로
  - 재입력 시 수신함 "처리 기록 지우기" 필요 (localStorage 기반이라 SQL로 안 지워짐)
- [x] **PT 한 줄 → 세 항목 분리** ✅ (2026-09-20, 실제 파일로 확인)
  - **실제 형식은 예상과 달랐다**: PT는 세 줄로 오지 않는다. **한 줄**(`B1520 / 응고기능기본검사-프로트롬빈시간`)에
    문장결과로 `INR : 1.67 / PT : 19.4 / Percent : 47`이 같이 오고, 참고치도 같은 라벨 형식으로 온다
  - 고치기 전: 첫 숫자 하나(INR)만 남고 **초·퍼센트 값이 사라졌다**. 이름도 한글 원문, 카테고리도 BC
  - [x] `splitCompositeResult()` — 라벨별로 나눠 `PT (INR)` / `PT (sec)` / `PT (%)` 세 항목 생성
  - [x] 행 안의 참고치를 항목별로 붙이고 H/L도 그 값으로 판정 (INR 1.67 → H, Percent 47 → L)
  - [x] `labCodeMap`에 `B1212: PT (sec)` 신설, 카테고리 Coagulation
  - [x] 아는 라벨이 없으면 **손대지 않는다** — 모르는 형식을 추측해서 쪼개지 않음
- [x] **Culture 접두사 유실 수정** ✅ (2026-09-20) — **VRE 직장도말이 CRE로 기록되고 있었다**
  - 원인: `resolveCultureName()`이 검체 종류만 보고 이름을 **CRE로 고정**해 만들고 있었다.
    `VRE-Rectal Swab` → 코드 미등록 → `normalizeName()`이 `-` 뒤 `Rectal Swab`만 떼어
    부분일치로 `CRE-Rectal swab`을 찾아 붙였다. 균이 Enterococcus faecium(VRE)인데 CRE로 남는다
  - [x] 접두사(CRE/VRE/MRSA/MRAB/ESBL/CPE)를 **들어온 그대로 유지**하도록 변경 — 격리·항생제 판단이 달라지는 임상 오류
  - [x] `b4112R: VRE-Rectal swab` 코드 등록
- [x] 회귀 테스트 5건 — 실제 파일에서 확인한 행 모양을 재현 (환자 식별 정보 없이)
- [ ] **긴 한글 검사명 매핑** — 다른 항목들도 원문이 그대로 남는지 재입력하며 확인
  - 원인 ①: **XLS 경로는 `extractCleanName()`을 쓰지 않는다.** `labParser.normalizeName()`이
    마지막 `-` 뒤 조각(`프로트롬빈시간`)만 떼어 `findLabByName()`에 넘기고, 실패하면 **원문 전체**를 반환한다
  - 원인 ②: 두 경로 모두 **한글 이름 → 짧은 이름** 매핑 단계가 아예 없다
  - [x] **사용자 결정 (2026-09-20): PT는 sec · % · INR 세 줄을 모두 따로 저장한다.**
    하나로 합치지 않는다 — 서로 다른 값이라 같은 이름으로 겹치면 추이가 망가진다
    → `PT (sec)` 항목을 `labCodeMap`에 신설해야 한다 (현재 `PT (INR)` B1210 · `PT (%)` B1211만 있음)
  - ✅ 해결됨 — 실제 파일을 받아 형식을 확인하고 위의 `splitCompositeResult()`로 처리
- [ ] **Culture 추가 확인** — VRE 케이스는 고쳤다. 다른 균·검체 조합(MRSA, 객담, 상처 등)이
  실제로 어떤 이름으로 오는지는 아직 표본이 없다. 재입력하며 이상한 이름이 보이면 추가
- [ ] 매핑 추가가 쉬운 구조로 정리 + 회귀 테스트 (사례 하나 = 테스트 하나)

### 2.10 LLM 모델 목록 갱신 `@Coder-Logic` — ✅ **완료 (2026-09-21)**
...
- [ ] 실사용 확인 — 설정 > AI 설정에서 **연결 테스트** 한 번씩 (키가 해당 모델을 지원하는지)

### 2.11 SOAP 초안 A) 섹션 개선 `@Coder-Logic` `@Coder-UI` — ✅ **완료 (2026-09-21)**

> **배경 (사용자)**: A)를 줄글로 쓰지 말고 Problem List 형식으로. 의심 진단은 `R/O`로 표기.
> 차트에 적어 둔 Problem List에서 골라 A)에 바로 넣을 수 있어야 한다.

- [x] 프롬프트 — A)는 한 줄에 하나 `#. 진단명`, 의심은 `#. R/O 진단명`
  - 예) `Urinary Tract Infection (UTI) 의심` → `#. R/O Urinary Tract Infection` (괄호 약어 병기는 생략)
  - 차트 Problem List에 있는 문제는 **그 표기 그대로** (`#. HTN`)
  - 근거 없는 진단 추가 금지는 기존 규칙 유지
- [x] `buildSoapContext()`에 환자 `problemList` 추가 — AI가 기존 표기를 재사용할 수 있게
- [x] **Problem List 칩** — SOAP 초안 위에 차트 Problem List가 버튼으로 뜨고, 누르면 A) 섹션 끝
  (P) 바로 앞)에 `#. HTN`이 들어간다. 중복 클릭은 무시
- [x] **AI 초안을 저장 전에 편집 가능**하게 변경 (`<pre>` → `<textarea>`)
  — AI가 쓴 문장을 손대지 못한 채 차트에 넣는 구조였다
- [x] 테스트 11건 — `workspaceData.test.ts`(8), `AiActionPanel.test.tsx`(3)
- [ ] 실사용 확인 — 실제 생성 결과가 `#.` / `R/O` 형식을 지키는지 (모델별로 다를 수 있음)

### 2.12 지시오더 (Standing Orders) `@Architect` `@Coder-UI` — ✅ **구현 완료 (2026-09-21)**

> **배경 (사용자)**: 처방할 때마다 넣는 지시오더를 환자 첫 화면(요약 탭)에 두고,
> 차팅처럼 템플릿으로 붙여넣을 수 있으면 좋겠다.

- [x] 마이그레이션 `202609210001_patient_standing_orders.sql` — `patients.standing_orders text not null default ''`
  - 차팅 필드들과 성격이 같아(환자당 현재 내용 하나) 별도 테이블 없이 컬럼으로
- [x] 도메인/뷰 타입 · 매퍼 · 리포지토리 컬럼 추가
- [x] **마이그레이션 미적용 대비** — 컬럼이 없으면 PostgREST가 `42703`으로 거절해
  **환자 조회 전체가 실패한다**. 그 경우 컬럼만 빼고 재조회하도록 했다
  (`withStandingOrdersFallback`). 환자 추가도 빈 값이면 컬럼을 아예 안 보낸다
- [x] 요약 탭 상단에 `StandingOrdersSection` — 편집·저장·복사 + **차팅과 같은 템플릿 팝업**
  (`TemplateField`에 `standingOrders` 추가, 템플릿 테이블은 `field text`라 스키마 변경 불필요)
- [x] 미저장 상태 표시 + 탭 이탈 경고 연동, Ctrl/⌘+Enter 저장
- [x] 테스트 4건 — `StandingOrdersSection.test.tsx`
- [ ] **사용자 액션**: 마이그레이션 적용 필요. 적용 전에는 지시오더 저장만 실패한다(나머지는 정상)

### 2.18 환자 명단 맨 아래 '퇴원'이 화면 밖으로 잘리던 문제 `@Coder-UI` — ✅ **수정 (2026-09-25)**

> **증상 (사용자)**: 좌측 환자 명단 맨 아래 '퇴원' 줄이 화면 아래로 내려가 있어 누르기 힘들다.

- [x] 원인: 모바일 환자 목록 드로어가 "환자 목록" 머리줄(`h-12`) 아래에 `PatientRail`을 `h-full`로 넣고 있었다.
  명단이 드로어 높이 전체를 차지해 **머리줄 높이(48px)만큼 바닥이 화면 밖으로 밀렸고**, 그 자리에 퇴원 줄이 있었다
- [x] 드로어 안 명단을 `flex-1 h-auto`로 — 머리줄을 뺀 남은 높이만 차지한다 (`AppShell.tsx`)
- ~~퇴원 머리줄 바닥 고정(sticky)~~ — 먼저 이렇게 배포했으나 **원인이 아니어서 되돌림** (사용자: 따로 고정할 필요 없음)

### 2.17 Culture 결과 보기 개선 `@Coder-UI` — ✅ **완료 (2026-09-25)**

> **배경 (사용자)**: No growth는 보기 편한데, 균이 동정된 결과는 ① 결과가 길어 검사명이 `C.`처럼 잘려
> 무슨 검사인지 모르겠고 ② 균·감수성이 한 줄로 이어져 읽기 어렵다. ③ 가능하면 결과보고일도 같이 보고 싶다.

- [x] `cultureText.ts` — 문장결과를 머리말(`<Culture & ID>` + 균 이름) / 항생제 한 줄씩(약제 · S/I/R · MIC) / 기타 텍스트로 나눔
  - 줄바꿈 없이 한 줄로 이어져 와도 같은 모양으로 복구. 모르는 줄은 버리지 않고 텍스트로 남김
  - 감수성 구간 밖에서는 MIC 괄호까지 있어야 판정으로 인정 (`Streptococcus group B` 오인 방지)
- [x] `CultureResultItem` — 한 줄짜리(No growth)는 기존처럼 이름 옆에, 긴 결과는 **이름을 위에 전체 폭으로** 두고 아래에 정렬
  - R은 빨강, I는 주황 강조. 날짜 라벨은 `접수 YYYY-MM-DD`로 명시
- [x] 테스트 8건 — `cultureText.test.ts`(6), `CultureSection.test.tsx`(2)
- ~~결과보고일~~ — **사용자 결정 (2026-09-25): 포함하지 않는다.** 이원 XLS에 보고일 컬럼이 없음(접수일자 `[6]`만 있음)

### 2.16 메모 탭이 지난 메모를 못 보여주던 문제 `@Coder-UI` — ✅ **수정 (2026-09-21)**

> **증상 (사용자)**: 어제 넣은 메모가 오늘 워크스페이스 메모 탭에서 아예 안 보인다.
> (환자 목록 기준일을 어제로 바꾸면 그 환자들은 잘 잡힌다 — 데이터는 있는데 화면이 못 보여준 것)

- [x] 원인: 메모 탭이 **Today 브리핑(`data`)에서 오늘 메모만** 읽고 있었다.
  `useNoteStore.fetchNotesByPatient()`는 있었지만 **UI에서 쓰는 곳이 없었다**
- [x] 탭 진입 시 환자 전체 기간 메모를 불러오도록 변경 (단계적 로딩 원칙대로 탭 진입 시점)
- [x] **날짜별 그룹 표시** — 최근 날짜가 위, 각 묶음에 `2026-09-20 · 어제 · 3` 헤더
- [x] 알림(reminder)은 **알림 날짜 기준**으로 묶는다 — "언제 띄울 것인가"가 본래 의미라
  적어 둔 날이 아니라 뜨는 날에 있어야 찾는다
- [x] AI SOAP 컨텍스트도 전체 메모에서 최근 5건을 쓰도록 자연히 개선됨 (오늘 메모가 없어도 동작)
- [x] 테스트 10건 — `NotesTab.test.tsx`(6), `workspaceData.test.ts` 그룹핑(4)
- [x] **최근 10개만 펼치고 나머지는 "더 보기"** (2026-09-21 사용자 요청)
  - 날짜 묶음은 쪼개지 않는다 — 같은 날 메모가 중간에서 잘리면 그 날 전부인지 알 수 없다
- [x] **메모 날짜 지정** (2026-09-21 사용자 요청) — 입력 줄에 날짜 칸, 기본값 오늘.
  지난 날짜로 적으면 그 날짜 묶음에 들어간다. 알림은 알림 날짜도 같이 그 날짜로
  - 🐛 **함께 고친 것**: `useNoteStore.addNote`가 `date`를 받아도 **화면에만** 반영하고
    서버에는 오늘로 저장하고 있었다(새로고침하면 날짜가 되돌아감). 이제 `created_at`으로 저장한다
  - Today 브리핑 낙관적 갱신은 **오늘 날짜일 때만** — 지난 날짜 메모가 오늘 큐에 끼면 안 된다
  - [x] 테스트 8건 추가 — 접기(2) · 날짜 지정(2) · `limitNoteGroups`(4)

### 2.15 환자 목록 기준일 바꾸기 `@Coder-Logic` `@Coder-UI` — ✅ **완료 (2026-09-21)**

> **배경 (사용자)**: 할 일·메모 필터가 늘 오늘 기준이다. 어제 메모가 있는 환자를 뽑아 보고 싶다.

- [x] 환자 목록 필터 아래 **기준일 줄** — 날짜 입력 + 하루 전/후 버튼 + `오늘` 되돌리기
- [x] `fetchDayScopedNotes(patientIds, date)` — 그 날짜의 메모·알림·일정만 조회.
  Today 브리핑을 통째로 다시 부르지 않는다
- [x] **오늘일 때는 추가 쿼리 없음** — 이미 받아 둔 브리핑 인디케이터를 그대로 쓴다.
  날짜를 옮긴 순간에만 조회한다
- [x] **항생제·Lab은 제외** — "지금 상태"라 과거 날짜에 붙이면 뜻이 흐려진다.
  기준일을 옮기면 화면에 그렇게 안내한다(할 일 = 알림·일정)
- [x] 날짜를 빠르게 바꿀 때 **늦게 온 응답이 최신 것을 덮지 않도록** 요청 순번 확인
- [x] 테스트 13건 — `useRailBasisDate.test.ts`(5), `PatientRail.test.tsx`(5), `patientIndexes.test.ts`(3)

### 2.14 환자별 중요사항 박스 `@Coder-UI` — ✅ **완료 (2026-09-21)**

> **배경 (사용자)**: 지시오더 위에 환자별 중요사항을 적어 두고 보는 칸. 템플릿·복사는 불필요, 저장만.

- [x] 마이그레이션 `202609210003_patient_important_notes.sql` — `patients.important_notes`
- [x] 요약 탭 최상단 배치 (중요사항 → 지시오더 → 필요열량 순)
- [x] **`StandingOrdersSection`을 `PatientTextSection`으로 일반화** — 두 박스가 거의 같은 코드였다.
  템플릿(`templateField`)과 복사(`copyTitle`)는 필요한 박스에서만 켠다
- [x] 테스트 5건 — `PatientTextSection.test.tsx` (템플릿·복사 없는 형태 포함)

### 2.13 1일 필요열량 계산기 `@Coder-Logic` `@Coder-UI` — ✅ **완료 (2026-09-21)**

> **배경 (사용자)**: 영양 처방 낼 때만 쓰는 계산이라 평소엔 접어 두고, 스위치를 켠 환자에서만 입력받는다.

- [x] `features/nutrition/calorieNeeds.ts` — Mifflin-St Jeor + 계수 표 (순수 함수)
  - 남 `10W + 6.25H − 5A + 5` / 여 `10W + 6.25H − 5A − 161`
  - 1일 필요열량 = REE × 활동계수 × 상해계수
  - **BMI > 30이면 보정체중** `IBW + 0.25 × (ABW − IBW)`, `IBW = 키(m)² × 22(남)/21(여)`
  - 나이·성별은 환자 기본정보에서, 키·체중만 입력받는다
  - 계수 값이 겹치므로(소수술·감염 둘 다 1.2) 선택 키는 `id`로 따로 둠
  - 패혈증은 1.4~1.6 범위라 1.4/1.5/1.6 세 항목으로 나눔
  - **범위를 벗어난 입력은 계산하지 않는다** (키 50~250cm, 체중 1~400kg) — 오타로 나온 열량이 그대로 처방되지 않도록
- [x] 요약 탭에 `CalorieNeedsSection` — 토글 스위치, 입력칸, 결과, **계산 근거 펼치기**
  - 근거에는 대입한 숫자가 그대로 보인다(`REE = 10 × 65 + 6.25 × 170 − 5 × 70 + 5 = 1368 kcal`)
  - 활동/상해 계수 표도 함께 펼쳐 현재 선택을 강조
  - 결과 + 근거를 한 번에 복사 가능
- [x] 환자를 바꾸면 입력값·스위치를 초기화 (이전 환자의 키·체중이 남지 않도록)
- [x] 테스트 20건 — `calorieNeeds.test.ts`(13), `CalorieNeedsSection.test.tsx`(7)
- [x] **환자별 저장** ✅ (2026-09-21, 사용자 요청) — 마이그레이션 `202609210002_patient_nutrition.sql`
  - `height_cm` / `weight_kg` (계산기 밖에서도 쓸 값이라 각자 컬럼) +
    `nutrition_enabled` / `nutrition_activity_factor` / `nutrition_injury_factor`
  - 환자를 열면 저장된 값으로 박스가 켜진 채 열리고, 미저장 표시 + 저장 버튼은 차팅과 같은 방식
  - **마이그레이션 미적용 대비 일반화** — `withMissingColumnFallback()`이 PostgREST 오류 문구에서
    없는 컬럼 이름을 읽어 **그 컬럼만** 빼고 재조회한다(최대 6회). 기존 `standing_orders` 전용
    폴백은 컬럼 하나만 알던 구조라, 컬럼이 늘어날 때마다 고치지 않아도 되게 바꿨다
  - 쓰기도 값이 `undefined`면 컬럼을 보내지 않아, 미적용 DB에서 다른 저장이 막히지 않는다
  - [x] 테스트 7건 — `missingColumnFallback.test.ts` (실제 DB 없이 폴백 동작 검증)
...
> "이몽룡님 소듐 요즘 어땠지?" 같은 음성 질문에 기존 Lab/투약 데이터를 조회해 텍스트+그래프로 답한다.
...
- [x] **환자명 발음 매칭 (오인식 보정)** ✅ (2026-09-20) — 실사용 피드백: "홍길돔"까지 인식했는데도 명단의 "홍길동"을 못 찾음
  - [x] `src/lib/koreanName.ts` — 한글 자모(초/중/종성) 단위 가중 편집거리. 혼동 자모(받침 ㄴ·ㅇ·ㅁ, 모음 ㅐ·ㅔ 등)는 낮은 비용으로 본다
  - [x] 후보가 둘 이상 비슷하게 가까우면 매칭하지 않는다(누군지 알 수 없음) · 두 글자 이름은 더 엄격하게
  - [x] LLM 출력에 `heardName`(들린 이름 그대로) 추가 — LLM이 `patientName`을 null로 두어도 코드가 발음으로 한 번 더 대조
  - [x] 유사 매칭은 `nameMatch: 'similar'`로 표시 → 간호사 대화 검토 UI에서 **기본 선택 해제 + "○○으로 들렸습니다, 확인해주세요"**, 음성 질의 답변에는 배너 표시
  - [x] 매칭 실패 시 들린 이름을 그대로 보여줘 왜 못 찾았는지 알 수 있게 함
  - [x] 테스트 14건 — `koreanName.test.ts`(11) + 대화/음성 정규화 테스트 갱신
...
| 2026-09-20 | **환자명 발음 매칭 추가**: 실사용에서 "홍길동"을 "홍길돔"으로 인식한 경우 LLM이 명단 철자를 못 맞히면 그대로 미지정으로 떨어졌다. `src/lib/koreanName.ts`에 자모 단위 가중 편집거리 매칭을 넣어 코드에서 한 번 더 대조한다. **명단에 있는 이름만 통과하는 환각 차단은 그대로 유지**하고, 유사 매칭은 `nameMatch: 'similar'`로 구분해 사람 확인을 요구한다(간호사 대화는 기본 선택 해제, 음성 질의는 배너). 임계값(3글자 0.8 / 2글자 0.3, 1·2등 차이 0.35 미만이면 매칭 포기)은 실사용에서 조정 필요. | ✅ 완료 | @Coder-Logic |
| 2026-09-20 | **회진 모드 추가 (2.6)**: 회진 중 좌측 환자 레일이 작아서 안 읽힌다는 사용자 피드백. 병동(병실번호 앞자리) 탭 → 병실 카드 → 이름 20px + 색·글자 플래그 뱃지 구조로 만들었다. 병동 목록은 하드코딩하지 않고 실제 병실번호에서 파생하며, 번호를 못 읽는 환자는 `기타`로 모은다. 환자를 열었다 돌아오면 보던 병동을 유지한다. **실제 병실 표기 체계는 확인 필요** — `parseRoomBed()`는 첫 숫자 덩어리를 병실, 그 뒤 숫자를 병상으로 본다. | ✅ 완료 | @Coder-UI + @Coder-Logic |
| 2026-09-20 | **회진 뱃지에 숫자 + 임계값 지속 표시**: 사용자 요청 — 플래그에 값이 보이고, 임계값 문제는 "다음 Lab에서 해결될 때까지" 계속 떠 있어야 한다. `alert_events`는 발생 기록이라 이 용도에 맞지 않아 `findOpenLabBreaches()`를 새로 만들었다(항목별 마지막 검사만 판정 → 값이 돌아오면 자동 소멸). 뱃지 문구는 브리핑 집계를 조합할 뿐이라 **추가 쿼리가 없다**. 한계: 판정 범위가 최근 14일이라, 마지막 검사가 그보다 오래됐으면 뜨지 않는다(평가용 조회 범위와 공유 — 넓히면 과거 알림이 한꺼번에 생성됨). | ✅ 완료 | @Coder-Logic + @Coder-UI |
...
| 2026-09-21 | **SOAP A) 섹션 Problem List 형식 (2.11)**: 프롬프트에 `#. 진단명` / 의심은 `#. R/O 진단명` 규칙 추가하고, 차트 Problem List를 컨텍스트로 넘겨 같은 문제는 기존 표기를 재사용하게 했다. UI로는 Problem List 칩을 눌러 A) 섹션 끝(P) 앞)에 바로 삽입한다. **이 과정에서 AI 초안이 읽기 전용이던 것을 편집 가능하게 바꿨다** — 생성문을 손대지 못한 채 차트에 저장하는 구조였다. | ✅ 완료 | @Coder-UI + @Coder-Logic |
| 2026-09-21 | **지시오더 칸 추가 (2.12)**: 요약 탭 상단에 환자별 지시오더를 두고 차팅과 같은 템플릿 팝업을 붙였다. 저장 위치는 `patients.standing_orders` 컬럼 — 차팅 필드들과 성격이 같아 별도 테이블을 만들지 않았다. **마이그레이션 미적용 DB에서 컬럼을 요청하면 환자 조회 전체가 42703으로 실패**하므로, 그 경우 컬럼을 빼고 재조회하는 폴백을 넣었다(환자 추가도 빈 값이면 컬럼을 안 보낸다). 즉 마이그레이션을 늦게 적용해도 앱은 멀쩡하고 지시오더 저장만 안 된다. | ✅ 완료 | @Architect + @Coder-UI |
| 2026-09-21 | **1일 필요열량 계산기 (2.13)**: Mifflin-St Jeor + 활동/상해 계수, BMI 30 초과 시 보정체중. 요약 탭에 토글로 열리는 박스로 넣고 계산 근거를 펼쳐 볼 수 있게 했다. **입력값은 저장하지 않는다** — 키·체중은 임상 데이터라 localStorage에 두면 "브라우저 저장소는 UI 편의 상태만"이라는 프로젝트 규칙을 어긴다. 저장하려면 컬럼 추가가 필요해 사용자 판단으로 남겨 둠. | ✅ 완료 | @Coder-Logic + @Coder-UI |
| 2026-09-21 | **필요열량 입력값 환자별 저장**: 키·체중·계수·박스 on/off를 `patients` 컬럼으로 저장. 키·체중은 계산기 밖에서도 쓸 값이라 각자 컬럼으로 뒀다. 이 과정에서 `standing_orders` 전용이던 미적용-마이그레이션 폴백을 **일반화**했다 — PostgREST 오류 문구에서 없는 컬럼 이름을 읽어 그 컬럼만 빼고 재조회한다. 전용 폴백은 컬럼이 늘 때마다 손봐야 했고, 전부 빼버리면 이미 적용된 마이그레이션의 값까지 잃는다. | ✅ 완료 | @Architect + @Coder-UI |
| 2026-09-21 | **환자별 중요사항 박스 (2.14)**: 요약 탭 최상단에 저장 전용 텍스트 박스. 지시오더 박스와 코드가 거의 같아 `PatientTextSection`으로 합치고, 템플릿·복사는 프롭으로 켜도록 했다 — 박스가 하나 더 늘 때 또 복제하지 않도록. | ✅ 완료 | @Coder-UI |
| 2026-09-21 | **환자 목록 기준일 (2.15)**: 할 일·메모 필터를 어제 등 다른 날짜 기준으로 볼 수 있게 했다. 오늘일 때는 브리핑 인디케이터를 그대로 써서 **추가 쿼리가 없고**, 날짜를 옮긴 순간에만 그 날짜의 메모·알림·일정을 조회한다. **항생제·Lab은 기준일에서 제외** — 날짜에 묶이는 신호가 아니라 지금 상태라, 과거 날짜에 붙이면 의미가 흐려진다(화면에 안내 표시). | ✅ 완료 | @Coder-Logic + @Coder-UI |
| 2026-09-21 | **메모 탭 지난 메모 누락 수정 (2.16)**: 메모 탭이 Today 브리핑에서 오늘 메모만 읽고 있어, 어제 적은 메모가 환자 화면에서 사라져 보였다(데이터는 정상). 탭 진입 시 환자 전체 메모를 불러와 날짜별로 묶어 보여준다. `useNoteStore.fetchNotesByPatient()`는 이미 있었지만 **UI에서 아무도 호출하지 않던 상태**였다 — 스토어만 만들어 두고 화면을 브리핑에 붙여 둔 흔적. 알림은 알림 날짜 기준으로 묶는다. | ✅ 완료 | @Coder-UI |
| 2026-09-21 | **메모 목록 접기 + 날짜 지정**: 최근 10개만 펼치고 나머지는 "더 보기"(날짜 묶음은 쪼개지 않음). 메모 추가 시 날짜를 골라 지난 날짜로도 넣을 수 있게 했다. 이 과정에서 **`addNote`의 날짜 지정이 화면에만 반영되고 서버에는 오늘로 저장되던 버그**를 발견해 함께 고쳤다(`created_at`을 실제로 보낸다). | ✅ 완료 | @Coder-UI + @Coder-Logic |
| 2026-09-21 | **로고 클릭 → Today (사용자 요청)**: Today 대시보드가 사실상 홈이라 로고를 눌러 돌아가려는 습관이 생기는데, 로고는 `Link to="/"`뿐이었다. 앱은 이미 `/`에 있어 라우터가 아무 것도 하지 않아 환자 워크스페이스가 그대로 남았다. 클릭 시 `onToday`를 부르도록 하고, **미저장 변경으로 막히면 주소 이동도 막는다**. ⌘/Ctrl·가운데 클릭(새 탭)은 가로채지 않는다. | ✅ 완료 | @Coder-UI |
| 2026-09-25 | **Culture 결과 보기 개선 (2.17)**: 균이 동정된 결과가 길어 검사명이 `C.`로 잘리고 감수성이 한 줄로 이어지던 문제. 문장결과를 머리말/항생제 한 줄씩(약제·S/I/R·MIC)으로 나눠 정렬하고, 긴 결과는 검사명을 위에 전체 폭으로 둔다. 표시만 바꿔 기존 데이터에도 바로 적용. **보고일은 표시하지 않기로 결정** — XLS에 컬럼이 없다. | ✅ 완료 | @Coder-UI |
| 2026-09-25 | **환자 명단 바닥 잘림 (2.18)**: 모바일 드로어가 머리줄 아래에 명단을 `h-full`로 넣어 48px만큼 화면 밖으로 밀렸고, 거기에 퇴원 줄이 있었다. 처음엔 퇴원 줄을 sticky로 고정하는 식으로 증상만 가렸다가 되돌렸다 — **목록 바닥이 안 보이면 먼저 컨테이너 높이 계산을 볼 것.** | ✅ 수정 | @Coder-UI |
