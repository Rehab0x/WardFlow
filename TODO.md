...
4. **이원의료재단 XLS 샘플 1개** — Culture 오분류·PT 세 줄 매핑을 고치려면 실제 파일이 필요하다 (2.9)
...
- [ ] **긴 한글 검사명 매핑** — `응고기능기본검사-프로트롬빈시간` 같은 이름이 그대로 저장됨
  ...
  - [ ] **막힌 지점: 실제 XLS 샘플 1개 필요.** 이원 XLS에는 **단위 컬럼이 없어서**
    (컬럼: 병원검사코드/이원코드/검사명/문자결과/문장결과/H·L/참고치) sec·%·INR을
    **검사명 문자열로만** 구분해야 한다. 세 줄이 실제로 어떤 이름으로 오는지 확인 전에는
    추측으로 매핑하지 않는다 — PT(초)를 INR로 잘못 붙이면 임상적으로 위험하다
- [ ] **Culture 오분류** — 재현 사례 필요. 원본 XLS 또는 잘못 들어간 행(이원코드/검사명/문자결과/문장결과)
