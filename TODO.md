...
### 🔴 사용자 액션 필요
0. **마이그레이션 적용** — `supabase/migrations/202609210001_patient_standing_orders.sql`
   (지시오더 칸. 적용 전에는 저장만 실패하고 나머지 기능은 정상 동작한다)
...
- [ ] **입력값은 저장하지 않는다** — 브라우저 저장소는 UI 상태 전용이라는 프로젝트 규칙 때문.
  환자별로 남기려면 `patients`에 컬럼 추가(마이그레이션)가 필요하다. 사용자 판단 대기
