...
> ⚠️ 이 문서는 시간순 누적 로그입니다. **가장 최신 상태는 맨 아래 "Current 2026-09-21 Checkpoint"를 먼저 읽으세요.**
...
- Patient delete policy now uses Supabase hard delete for the v2 app. The edit panel labels the action as patient delete, the confirmation explains that linked clinical records are deleted together, and `patientDeletionPolicy` plus Supabase patient-store tests cover the behavior.
...
- Patient delete wording now uses a shared failure message in the v2 patient edit flow and patient store, and the Supabase patient-store test covers delete failure without removing the patient locally.
...
- v2 patient registration now allows multiple patients in the same room/bed, patient deletion is wired to Supabase hard delete through a new delete RLS policy migration, the mobile workspace header lets C/C and onset use full width below the action row, Lab now has a per-patient value table, and charting OCS copy/template access was restored in the v2 workspace.
...
4. Apply `supabase/migrations/202605270001_allow_patient_delete.sql` to enable RLS-protected hard delete for v2 patient deletion in the live Supabase project.
...
## Current 2026-09-20 (오후) Checkpoint — 실사용 피드백 반영

> 앞 체크포인트(2026-09-19)는 기능을 "만든" 세션이고, 이 세션은 **실제로 써 보고 고친** 세션이다.
> 다섯 개를 배포했다: 환자명 발음 매칭 · 회진 모드 · 뱃지 숫자/임계값 지속 · 메모 필터 · 앱 아이콘.

### 배포된 커밋 (전부 main, Vercel 자동 배포 확인)

| 커밋 | 내용 |
|------|------|
| `e14e76b` | 환자명 발음 매칭 — "홍길돔"을 명단의 "홍길동"으로 잇는다 |
| `e55171d` | 회진 모드 — 병동 탭 + 병실 카드 + 큰 이름 |
| `7df47e9` | 회진 뱃지에 숫자, 임계값 위반은 해결될 때까지 유지 |
| `f836cbc` | 환자 목록 '메모' 필터 — 오늘 경과기록 있는 환자 |
| `b3cf503` | 앱 아이콘 재디자인 (맥박이 그리는 W) |

### 설계 판단 메모 (다음 세션이 뒤집기 전에 읽을 것)

- **유사 매칭은 자동 저장하지 않는다.** `matchRosterName()`은 정확 일치(`exact`)와 발음 유사
  (`similar`)를 구분해 돌려준다. 유사는 간호사 대화 검토에서 **체크 해제 상태**로 시작하고,
  음성 질의는 배너로 알린다. 환자 오배정은 되돌리기 어려워서 클릭 한 번을 일부러 남겼다.
  후보가 둘 이상 비슷하게 가까우면 **매칭을 포기**한다(누군지 알 수 없다는 뜻).
- **`alert_events`와 회진 뱃지는 다른 질문에 답한다.** 이벤트는 "언제 터졌나"의 기록이라
  값이 정상으로 돌아와도 남는다. 뱃지는 "지금도 문제인가"여야 하므로
  `findOpenLabBreaches()`가 **항목별 마지막 검사**만 보고 따로 판정한다.
  그 항목을 재지 않은 검사는 건너뛴다 — Lab을 안 냈다고 문제가 없어지지 않는다.
  ⚠️ 판정 범위는 알림 평가와 공유하는 **최근 14일**(`LAB_LOOKBACK_DAYS`)이다.
  넓히면 과거 알림이 한꺼번에 생성되므로 분리 없이는 늘리지 말 것.
- **회진 뱃지는 추가 쿼리를 하지 않는다.** 문구는 이미 불러온 브리핑 집계를 조합할 뿐이다
  (`buildRoundingBadges`). 첫 화면 성능 규칙을 지키려면 이 성질을 깨지 말 것.
- **'메모'는 '할 일'에 섞지 않는다.** 메모는 이미 한 일, 할 일은 남은 일이라 섞으면 둘 다
  의미가 흐려진다. 카운트도 따로 센다.
- **아이콘은 플랫폼별로 세 변형이 필요하다.** 브라우저/PWA는 우리가 모서리를 둥글리고,
  iOS는 자체 마스크가 있어 정사각이어야 하며, 안드로이드 적응형은 바깥 20%가 잘려 마크를
...
**막힌 지점: 실제 XLS 샘플 1개가 필요하다.**

- 이원 XLS 컬럼: `[8] 병원검사코드 [9] 이원코드 [10] 검사명 [11] 문자결과 [12] 문장결과 [13] H/L [15] 참고치`
  → **단위 컬럼이 없다.** 단위는 코드맵에서 온다.
- `응고기능기본검사-프로트롬빈시간`이 그대로 저장되는 원인:
  **XLS 경로는 `extractCleanName()`을 쓰지 않는다.** `labParser.normalizeName()`이
  마지막 `-` 뒤 조각만 떼어 `findLabByName()`에 넘기고, 실패하면 원문 전체를 반환한다.
  두 경로 모두 한글 이름 → 짧은 이름 매핑 단계가 없다.
- **사용자 결정: PT는 sec · % · INR 세 줄을 모두 따로 저장한다.** 합치지 않는다.
  `PT (sec)` 항목을 `labCodeMap`에 신설해야 한다(지금은 `PT (INR)` B1210, `PT (%)` B1211만).
  단위 컬럼이 없어 **검사명 문자열로만** 셋을 구분해야 하므로, 실제 이름을 보기 전에는
  추측으로 매핑하지 않는다 — PT(초)를 INR로 잘못 붙이면 임상적으로 위험하다.
- Culture 오분류도 재현 사례(이원코드/검사명/문자결과/문장결과)가 있어야 고친다.
- 사례 하나당 회귀 테스트 하나씩 붙일 것. 파싱 수정은 서로를 되돌리기 쉽다.

### Lab 초기화

`supabase/maintenance/reset-labs.sql` — SQL 에디터에서 실행한다(앱에는 버튼을 두지 않았다).
STEP 1로 건수를 먼저 확인하고, 2-A(전체) 또는 2-B(사용자 한정)의 주석을 푸는 구조다.
`lab_items`는 cascade로 함께 지워진다. 환자·메모·투약·차팅은 남는다.
재입력 전에 앱의 Lab 수신함에서 **"처리 기록 지우기"**를 눌러야 한다
(처리 완료 표시가 localStorage에 있어 SQL로는 안 지워진다).

### 검증 (2026-09-20 오후 최종)

- `npm run type-check` / `npm run lint`(에러 0, 경고 15) / `npm run build` 통과
- `npx vitest run` — 50 files / **339 tests** 통과 (직전 277 → +62)
- 프로덕션 배포 확인: https://ward-flow.vercel.app — 커밋별로 번들 해시 교체 확인

---

## Current 2026-09-21 Checkpoint — 실사용하며 다듬은 하루

> 이 세션은 **쓰면서 걸린 것들을 그 자리에서 고친** 기록이다. 새 기능 세 개(지시오더·필요열량·중요사항)와
> 버그 두 개(메모 탭·메모 날짜 저장), 나머지는 다듬기. 커밋 12개 전부 배포됨.

### 마이그레이션

**2026-09-21 기준 전부 적용 완료.** 대기 중인 것 없다.
이 세션에서 추가된 것: `202609210001`(지시오더) · `202609210002`(필요열량) · `202609210003`(중요사항).

### 설계 판단 메모 (다음 세션이 뒤집기 전에 읽을 것)

- **모델 ID는 제공사 공식 문서에서만 확인한다.** 가격 비교 사이트들이 은퇴한 모델을 계속 싣고 있다.
  이번에 `grok-4.1-fast`가 그랬다 — 2026-05-15 은퇴, 요청은 `grok-4.3`으로 리다이렉트되고 4.3 요금이
  청구된다. 목록에 넣으면 싼 모델을 쓰는 게 아니라 이름만 옛것이 된다.
  **가격은 코드에 적지 않는다** — 금방 틀린 정보가 된다.
- **마이그레이션 미적용 폴백은 "없다고 한 컬럼만" 뺀다**(`withMissingColumnFallback`).
  후보를 전부 빼면 이미 적용된 마이그레이션의 값까지 잃고, 그 상태로 저장하면 실제 값을 덮어쓴다.
  PostgREST 오류 문구에서 컬럼 이름을 읽어 그것만 제거하고 재조회한다. 컬럼이 늘어도 이 함수는 그대로다.
  쓰기도 값이 `undefined`면 컬럼을 보내지 않는다.
- **AI 초안은 저장 전에 고칠 수 있어야 한다.** SOAP 결과가 읽기 전용 `<pre>`였다 — AI가 쓴 문장을
  한 글자도 못 고친 채 차트에 저장하는 구조였다. `textarea`로 바꿨고, 여기에 Problem List 칩을 붙여
  A) 섹션에 `#. HTN`을 끼워 넣는다.
- **A) 섹션은 Problem List 형식**(`#. 진단명`, 의심은 `#. R/O 진단명`). 차트의 Problem List를
  컨텍스트로 넘겨 같은 문제는 기존 표기를 재사용하게 한다 — HTN과 Hypertension으로 갈라지지 않도록.
- **필요열량은 범위를 벗어난 입력을 계산하지 않는다**(키 50~250cm, 체중 1~400kg).
  170을 17로 잘못 친 결과가 그대로 처방에 들어가면 안 된다. BMI 30 초과는 보정체중을 쓴다.
- **환자 목록 기준일에 항생제·Lab은 넣지 않는다.** 그건 날짜에 묶이는 신호가 아니라 "지금 상태"다.
  어제 칸에 붙이면 무슨 뜻인지 알 수 없어진다. 대신 무엇을 보고 있는지 화면에 적는다.
- **메모 그룹에서 알림은 알림 날짜 기준**이다. "언제 띄울 것인가"가 본래 의미라 적어 둔 날이 아니라
  뜨는 날에 있어야 찾는다. 목록은 최근 10개까지 펼치되 **날짜 묶음은 쪼개지 않는다**.
- **Tailwind `grid`만 쓰면 암묵 열이 `auto`**라 긴 줄에 밀려 칸 밖으로 넘친다.
  `grid-cols-1`은 `minmax(0,1fr)`이라 넘치지 않는다. 불필요해 보여도 지우지 말 것.

### 이 세션에서 드러난 버그 둘 (둘 다 회귀 테스트로 고정)

1. **메모 탭이 Today 브리핑에서 읽고 있었다** — 브리핑에는 오늘 것만 있어, 어제 적은 메모가
   환자 화면에서 사라져 보였다. `useNoteStore.fetchNotesByPatient()`는 처음부터 있었는데
   화면에서 아무도 부르지 않았다. 스토어만 만들고 UI는 브리핑에 붙여 둔 자리였다.
2. **메모 날짜 지정이 화면에만 반영됐다** — `addNote`가 `date`를 받아 로컬 객체에만 쓰고
   서버에는 `now()`로 저장했다. 새로고침하면 날짜가 되돌아갔다. 이제 `created_at`으로 보낸다.

### 다음 세션이 이어서 할 일

- **Lab 파싱 — 남은 오류 수집(TODO 2.9)**. PT 패널 분리와 VRE 접두사는 고쳤다.
  다음은 실제로 이상하게 들어온 행을 받아야 한다: 다른 Culture 조합(MRSA·객담·상처),
  긴 한글 검사명이 그대로 남는 항목. **사례 하나당 회귀 테스트 하나.**
- 실사용 검증 항목은 TODO.md 🟡 표 참고 (알림 임계값, 음성 인식, 회진 뱃지 길이 등).
- Phase 4 WardLink 통합은 여전히 **방향 상의 필요**.

### 검증 (2026-09-21 최종)

- `npm run type-check` / `npm run lint`(에러 0, 경고 15) / `npm run build` 통과
- `npx vitest run` — 58 files / **427 tests** 통과 (직전 392 → +35)
- 프로덕션 배포 확인: https://ward-flow.vercel.app — 커밋별로 번들 해시 교체 확인
